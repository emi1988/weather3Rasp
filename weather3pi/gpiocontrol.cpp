...
int gpioControl::GPIOSetWatch(QList<int> pins)
{
...
void gpioControl::checkGpio()
{
    ...
    for (int i = 0; i < watchedGpioPins.size(); ++i)
    {
        ...
        if(GPIORead(watchedGpioPins.at(i)) == 1)
        {
            //emit gpioInputChanged(watchedGpioPins.at(i));

            emit gpioInputChanged(watchedGpioPins.at(i));

            qDebug() << "high Signal at pin: "<< QString::number(watchedGpioPins.at(i));
